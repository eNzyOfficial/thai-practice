...
const alphabet = ref(alphabetStore.shortLongCharacters);
const currentCharacter = computed(() => alphabet.value[currentIndex.value]);

function answerQuestion(answer) {
    cheat.value = false;

    if (answer === currentCharacter.value.is_short) {
...
function resetGame() {
    alphabet.value = shuffle.value
        ? alphabetStore.shortLongCharacters.slice().sort(() => Math.random() - 0.5)
        : alphabetStore.shortLongCharacters;
...
<template>
    <div class="min-h-screen flex flex-col">
        <van-nav-bar title="Character Short/Long" left-text="Back" left-arrow @click-left="backButton" />
        ...
        <div class="flex flex-col flex-grow p-4">
            ...
            cheat ?
                (currentCharacter.is_short ? 'short' : 'long') :
                'cheat'
        }}</span>
            </div>
            ...
            <van-space direction="vertical" fill v-if="!gameFinished">
                <van-button type="primary" block @click="answerQuestion(false)">Long</van-button>
                <van-button type="primary" block @click="answerQuestion(true)">Short</van-button>
            </van-space>
